...
impl From<cli::Opt> for Config {
    fn from(opt: cli::Opt) -> Self {
        ...
        // When minus/plus line counts match (typical replacement hunks), allow
        // word-diff highlighting even when the changed span sits near EOL and
        // the relative edit distance would otherwise exceed max_line_distance
        // (see https://github.com/dandavison/delta/issues/2231). Override with
