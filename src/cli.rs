...
use clap::{ArgMatches, ColorChoice, CommandFactory, FromArgMatches, Parser};
use console::Term;
...
#[command(
    ...
    max_term_width(TERM_FALLBACK_WIDTH),
)]
pub struct Opt {
...
fn get_after_long_help(term: &Term) -> String {
    ...
    let wrap = textwrap::Options::new(term_width)
...
"##
    ...
    textwrap::wrap(&after_long_help, wrap).join("\n")
}
    ...
    fn handle_help_and_version(args: &[OsString]) -> Call<ArgMatches> {
        match Self::command().try_get_matches_from(args) {
            ...
            Err(e) if e.kind() == clap::error::ErrorKind::DisplayHelp => {
                ...
                let help_text = if args.iter().any(|arg| arg == "-h") {
                    Self::command().render_help()
                } else {
                    // generate long help on demand:
                    Self::command()
                        .after_long_help(get_after_long_help(&term))
                        .render_long_help()
                };

                if term.is_term() {
                    Call::Help(format!("{}", help_text.ansi()))
                } else {
