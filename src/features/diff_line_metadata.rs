...
//
// This is gated on the EMIT_OSC1717_METADATA environment variable: the host
...
/// in `EMIT_OSC1717_METADATA` (e.g. "V1" or "V1,V2"), or `None` when no host is
...
pub fn negotiated_version() -> Option<u32> {
    ...
    *VERSION.get_or_init(|| {
        let advertised = std::env::var("EMIT_OSC1717_METADATA").ok()?;
        advertised
            .split(',')
