...
use crate::fatal;
use crate::git_config::GIT_CONFIG_KEYS_FOR_DELTA_REGEX;
    ...
    pub fn new(config: &Config) -> Self {
        fn make(logfile: &OsString, config: &Config) -> Result<Box<DeltaDetailInternal>, String> {
            ...
            if let Some(cfg) = config.git_config.as_ref() {
                write("relevant git config values:\n".into())?;
                cfg.for_each(
                    GIT_CONFIG_KEYS_FOR_DELTA_REGEX,
                    |entry, value: Option<&str>| {
            ...
            delta_env.current_dir = None; // redact, not interesting
            ...
            write("raw crash input to delta (usually something git diff etc. generated):\n".into())?;
            write("================================\n".into())?;
